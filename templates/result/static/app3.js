...
form5_ele.addEventListener("submit",async (event)=>{
    ...
    if(userChoice == "continue"){
        allow_edit = false;
        form5_ele.innerHTML = '<p style="font-size:30px"><b>'+min_ele.value+'分'+sec_ele.value+'秒</b></p>'
        ...
        fetch("/api/create_result_file",{
            ...
            body: JSON.stringify({content:String(document.documentElement.outerHTML),team:String(team_name_ele.textContent)})  // 複数の変数を送信
        });
    }
});
